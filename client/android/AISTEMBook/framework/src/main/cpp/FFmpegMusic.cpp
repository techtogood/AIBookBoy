...
bool b_create_FFmpeg = false;
//opensl es调用 int * rate,int *channel
int createFFmpeg(int *rate,int *channel,int*duration,const char* file){
    av_register_all();
    //char *input = "/storage/emulated/0/pauseRecordDemo/video/a.mp3";
    pFormatCtx = avformat_alloc_context();
    LOGE("createFFmpeg %s",file);
    LOGE("xxx %p",pFormatCtx);
    int error;
    char buf[] = "";
    //打开视频地址并获取里面的内容(解封装)
    if (error = avformat_open_input(&pFormatCtx, file, NULL, NULL) < 0) {
        av_strerror(error, buf, 1024);
        // LOGE("%s" ,inputPath)
        LOGE("Couldn't open file %s: %d(%s)", file, error, buf);
        // LOGE("%d",error)
        LOGE("打开视频失败")
    }
